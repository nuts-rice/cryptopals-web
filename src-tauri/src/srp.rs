...
pub mod srp_handshake {
    ...
    pub fn server_session(v: Vec<u8>, pub_keys: &mut PubKeys) -> Result<Vec<u8>, ()> {
        let mut rng = rand::rngs::OsRng;
    ...
    pub fn client_verify(pub_keys: PubKeys, secret: &Secret, proof: Vec<u8>, priv_a: &[u8]) {
        let client = SrpClient::<Sha256>::new(&G_2048);
        unimplemented!()
    }
}
...
mod tests {
    ...
    fn hash_test() {
        ...
        let pub_b = srp_handshake::server_session(hash, &mut pub_k);
